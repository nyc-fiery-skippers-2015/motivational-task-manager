#ZM: Your route has the user ID, but you never use the ID.
...
#ZM: You have the current user id in the session 
...
#ZM: You have the current logged in user
post '/users/:id/lists/new' do
  #CD: Add require_logged_in
  current_user = User.find_by(id: params[:id])
  #CD: The url: List.custom_url should be an ActiveRecord BeforeCreate call back
...
#  current_user.lists.first.id
  ...
  #ZM: What happens if the tasks does not save?
  new_task.save
  redirect "/users/#{current_user.id}"
end
